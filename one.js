const platformStats = {
  teachers: { value: 5280, suffix: '+', label: 'معلم يستخدم أصـــنعها' },
  materials: { value: 18460, suffix: '+', label: 'ملزمة وكتاب تم إنشاؤها' },
  hours: { value: 32700, suffix: '+', label: 'ساعة عمل تم توفيرها' }
};
  ...
  keys.forEach((key, i) => {
    ...
    card.innerHTML = `
      <div class="stat-number">
        <span class="stat-count" data-target="${stat.value}">0</span><span>${stat.suffix}</span>
      </div>
...
function setupCtaButtons(){
  document.querySelectorAll('[data-goto-login]').forEach(btn => {
